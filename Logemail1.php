...
$a=trim($_POST["e"]);
$b=trim($_POST["p"]);
$x="select * from ecsignupp1 where Name='$z' and  Email='$a' and BINARY Password='$b'";
$y=$conn->query($x);
if($y->num_rows>0)
{
	$row = $y->fetch_assoc();
...
mysqli_query($conn,"INSERT into signin1 values('$username','$password')");
echo "<script type='text/javascript'>alert('Signed In successfully..!!');</script>";
include "newhomepage.html";
exit();
}
elseif(empty($a&&$b&&$z))
{
